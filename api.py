"""JSON API + iframe player. Python 3.8 compatible."""
import logging
from typing import Optional

from fastapi import APIRouter, Depends, HTTPException, Request
from fastapi.responses import HTMLResponse, RedirectResponse
from pydantic import BaseModel
from sqlalchemy import func, or_, select
from sqlalchemy.ext.asyncio import AsyncSession

from app.auth.security import get_current_user
from app.models.anime import Anime, Episode
from app.models.user import Favorite, User, WatchProgress
from app.services.video_provider import video_provider
from database import get_session
from main_templates import templates

logger = logging.getLogger("animeflow.api")

router = APIRouter(prefix="/api")


def _alt_titles(anime: Anime):
    if not anime.alternative_titles:
        return []
    return [
        a.strip()
        for a in anime.alternative_titles.replace(";", ",").split(",")
        if a.strip()
    ]


def _episode_video_url(episode: Episode) -> Optional[str]:
    """Pick the best stored URL for an episode without going to the network."""
    if not episode:
        return None
    for attr in ("anilibria_hls_hd", "anilibria_hls_fhd", "anilibria_hls_sd"):
        try:
            url = getattr(episode, attr, None)
        except Exception:
            url = None
        if url:
            return url
    if getattr(episode, "video_url", None):
        return episode.video_url
    if getattr(episode, "anilibria_iframe", None):
        return episode.anilibria_iframe
    return None


async def _resolve_video_url(
    session: AsyncSession,
    episode: Optional[Episode],
    anime: Anime,
    episode_number: int,
) -> Optional[str]:
    """Stored URL first, fall back to a live lookup if missing."""
    direct = _episode_video_url(episode) if episode else None
    if direct:
        return direct
    try:
        return await video_provider.find_for_episode(
            anime.title,
            episode_number=episode_number,
            alt_titles=_alt_titles(anime),
            anilibria_id=getattr(anime, "anilibria_id", None),
            anilibria_code=getattr(anime, "anilibria_code", None) or None,
        )
    except Exception as exc:
        logger.warning("video_provider error anime=%r ep=%d: %s", anime.title, episode_number, exc)
        return None


# ---------------------------------------------------------------------------
#  Iframe player
# ---------------------------------------------------------------------------

@router.get("/player", response_class=HTMLResponse)
async def player_page(
    request: Request,
    episode_id: Optional[int] = None,
    anime_id: Optional[int] = None,
    redirect: int = 0,
    session: AsyncSession = Depends(get_session),
):
    episode: Optional[Episode] = None
    anime: Optional[Anime] = None
    episode_number = 1

    try:
        if episode_id is not None:
            episode = await session.get(Episode, episode_id)
            if episode is None:
                raise HTTPException(status_code=404, detail="Episode not found")
            anime = await session.get(Anime, episode.anime_id)
            episode_number = episode.episode_number

        if anime is None and anime_id is not None:
            anime = await session.get(Anime, anime_id)
            if anime is None:
                raise HTTPException(status_code=404, detail="Anime not found")

        if anime is None:
            raise HTTPException(status_code=400, detail="Нужно episode_id или anime_id")

        video_url = await _resolve_video_url(session, episode, anime, episode_number)
    except HTTPException:
        raise
    except Exception as exc:
        logger.error("player_page error: %s", exc)
        return templates.TemplateResponse(
            request,
            "player.html",
            {
                "video_url": None,
                "title": "Ошибка",
                "episode_number": 1,
                "anime_id": anime_id,
                "episode_id": episode_id,
                "not_found_reason": "Произошла внутренняя ошибка. Попробуйте обновить страницу.",
                "next_url": "",
            },
        )

    not_found_reason = None
    if not video_url:
        logger.info("player miss anime=%r ep=%d", getattr(anime, "title", "?"), episode_number)
        not_found_reason = "Видео ещё не загружено или источник временно недоступен."

    if video_url and redirect:
        return RedirectResponse(video_url, status_code=302)

    next_url = ""
    if episode is not None:
        try:
            next_q = await session.execute(
                select(Episode)
                .where(
                    Episode.anime_id == anime.id,
                    Episode.episode_number > episode.episode_number,
                )
                .order_by(Episode.episode_number.asc())
                .limit(1)
            )
            nxt = next_q.scalar_one_or_none()
            if nxt is not None:
                next_url = "/anime/{}/episode/{}".format(anime.id, nxt.episode_number)
        except Exception:
            pass

    return templates.TemplateResponse(
        request,
        "player.html",
        {
            "video_url": video_url,
            "title": getattr(anime, "title", ""),
            "episode_number": episode_number,
            "anime_id": getattr(anime, "id", None),
            "episode_id": episode.id if episode else None,
            "not_found_reason": not_found_reason,
            "next_url": next_url,
        },
    )


@router.get("/player/data")
async def player_data(
    episode_id: int,
    session: AsyncSession = Depends(get_session),
):
    try:
        episode = await session.get(Episode, episode_id)
        if episode is None:
            raise HTTPException(status_code=404, detail="Episode not found")
        anime = await session.get(Anime, episode.anime_id)
        if anime is None:
            raise HTTPException(status_code=404, detail="Anime not found")

        video_url = await _resolve_video_url(session, episode, anime, episode.episode_number)
    except HTTPException:
        raise
    except Exception as exc:
        logger.error("player_data error: %s", exc)
        raise HTTPException(status_code=500, detail="Внутренняя ошибка")

    kind = "none"
    if video_url:
        if ".m3u8" in video_url:
            kind = "hls"
        elif video_url.endswith((".mp4", ".webm", ".mkv")):
            kind = "video"
        else:
            kind = "iframe"
    return {
        "episode_id": episode.id,
        "anime_id": anime.id,
        "episode_number": episode.episode_number,
        "title": episode.title,
        "anime_title": anime.title,
        "video_url": video_url,
        "kind": kind,
        "iframe_url": "/api/player?episode_id={}&anime_id={}".format(episode.id, anime.id),
        "page_url": "/anime/{}/episode/{}".format(anime.id, episode.episode_number),
    }


@router.get("/iframe")
async def resolve_iframe(
    anime_id: Optional[int] = None,
    episode_number: int = 1,
    title: Optional[str] = None,
    session: AsyncSession = Depends(get_session),
):
    chosen_title: Optional[str] = title
    alts: list = []
    anilibria_id: Optional[int] = None
    anilibria_code: Optional[str] = None
    if anime_id is not None:
        anime = await session.get(Anime, anime_id)
        if anime is None:
            raise HTTPException(status_code=404, detail="Аниме не найдено")
        chosen_title = anime.title
        alts = _alt_titles(anime)
        anilibria_id = getattr(anime, "anilibria_id", None)
        anilibria_code = getattr(anime, "anilibria_code", None) or None
    if not chosen_title:
        raise HTTPException(status_code=400, detail="Нужно anime_id или title")
    try:
        url = await video_provider.find_for_episode(
            chosen_title,
            episode_number=episode_number,
            alt_titles=alts,
            anilibria_id=anilibria_id,
            anilibria_code=anilibria_code,
        )
    except Exception as exc:
        logger.warning("resolve_iframe error: %s", exc)
        url = None
    return {"url": url, "title": chosen_title, "episode_number": episode_number}


@router.get("/healthz")
async def healthz():
    return {"status": "ok"}


@router.get("/search")
async def live_search(
    q: str = "",
    limit: int = 12,
    session: AsyncSession = Depends(get_session),
):
    """SQL-backed search with Unicode fallback. Returns up to `limit` results."""
    q_raw = q.strip()
    if not q_raw:
        return {"results": []}

    limit = max(1, min(limit, 200))
    q_low = q_raw.lower()
    pattern = "%{}%".format(q_low)

    try:
        # Primary: SQL LIKE (fast for ASCII/Latin queries)
        result = await session.execute(
            select(Anime)
            .where(
                or_(
                    func.lower(Anime.title).like(pattern),
                    func.lower(Anime.alternative_titles).like(pattern),
                )
            )
            .order_by(Anime.rating.desc(), Anime.id.desc())
            .limit(limit)
        )
        matched = list(result.scalars().all())

        # If SQL missed results (Cyrillic case-fold gap), do Python fallback
        if len(matched) < limit:
            seen_ids = {a.id for a in matched}
            extra_q = await session.execute(
                select(Anime)
                .where(Anime.id.notin_(seen_ids))
                .order_by(Anime.rating.desc(), Anime.id.desc())
                .limit(2000)
            )
            for a in extra_q.scalars().all():
                if len(matched) >= limit:
                    break
                haystack = "{} {}".format(a.title or "", a.alternative_titles or "").lower()
                if q_low in haystack:
                    matched.append(a)

    except Exception as exc:
        logger.error("search error q=%r: %s", q_raw, exc)
        return {"results": [], "error": "Ошибка поиска"}

    return {
        "results": [
            {
                "id": a.id,
                "title": a.title or "",
                "year": a.year or None,
                "poster_url": a.poster_url or "",
                "rating": round(float(a.rating), 1) if a.rating else None,
                "status": a.status or "",
            }
            for a in matched
        ]
    }


class FavoriteToggle(BaseModel):
    anime_id: int


@router.post("/favorites/toggle")
async def toggle_favorite(
    payload: FavoriteToggle,
    session: AsyncSession = Depends(get_session),
    user: Optional[User] = Depends(get_current_user),
):
    if not user:
        raise HTTPException(status_code=401, detail="Войдите в аккаунт")
    try:
        existing = await session.execute(
            select(Favorite).where(
                Favorite.user_id == user.id, Favorite.anime_id == payload.anime_id
            )
        )
        fav = existing.scalar_one_or_none()
        if fav:
            await session.delete(fav)
            await session.commit()
            # Re-check achievements after removing favorite
            try:
                from app.services.achievements import check_and_unlock
                await check_and_unlock(user.id, session)
            except Exception:
                pass
            return {"favorited": False}
        session.add(Favorite(user_id=user.id, anime_id=payload.anime_id))
        await session.commit()
        # Check achievements after adding a favorite
        try:
            from app.services.achievements import check_and_unlock
            await check_and_unlock(user.id, session)
        except Exception:
            pass
        return {"favorited": True}
    except Exception as exc:
        logger.error("toggle_favorite error: %s", exc)
        raise HTTPException(status_code=500, detail="Ошибка при обновлении избранного")


class ProgressPayload(BaseModel):
    episode_id: int
    timestamp: float
    duration: float


@router.post("/progress")
async def save_progress(
    payload: ProgressPayload,
    session: AsyncSession = Depends(get_session),
    user: Optional[User] = Depends(get_current_user),
):
    if not user:
        return {"saved": False}
    try:
        result = await session.execute(
            select(WatchProgress).where(
                WatchProgress.user_id == user.id,
                WatchProgress.episode_id == payload.episode_id,
            )
        )
        progress = result.scalar_one_or_none()
        is_new = progress is None
        if progress:
            progress.timestamp = payload.timestamp
            progress.duration = payload.duration
        else:
            progress = WatchProgress(
                user_id=user.id,
                episode_id=payload.episode_id,
                timestamp=payload.timestamp,
                duration=payload.duration,
            )
            session.add(progress)
        await session.commit()

        # Trigger achievement checks after meaningful watch progress
        try:
            from app.services.achievements import check_and_unlock
            from datetime import datetime

            # Determine if episode is considered "watched" (>80% through)
            episode_watched = (
                payload.duration > 0
                and payload.timestamp > 30
                and payload.timestamp >= payload.duration * 0.8
            )

            # Fire episode_watched event when episode is substantially watched
            if episode_watched:
                await check_and_unlock(user.id, session, event="episode_watched")
            else:
                # Periodic progress check (no specific event, just recompute DB-based ones)
                await check_and_unlock(user.id, session)

            # Night marathon: check if current hour is between 2:00 and 5:00
            hour = datetime.utcnow().hour
            if hour >= 2 and hour < 5 and payload.timestamp > 30:
                await check_and_unlock(user.id, session, event="night_watch")

        except Exception as exc:
            logger.warning("achievement check after progress error: %s", exc)

        return {"saved": True}
    except Exception as exc:
        logger.warning("save_progress error: %s", exc)
        return {"saved": False}


@router.get("/episode/{episode_id}")
async def episode_data(
    episode_id: int,
    session: AsyncSession = Depends(get_session),
):
    try:
        result = await session.execute(select(Episode).where(Episode.id == episode_id))
        ep = result.scalar_one_or_none()
        if not ep:
            raise HTTPException(status_code=404, detail="Episode not found")
        return {
            "id": ep.id,
            "anime_id": ep.anime_id,
            "episode_number": ep.episode_number,
            "title": ep.title or "",
            "video_url": ep.video_url or "",
        }
    except HTTPException:
        raise
    except Exception as exc:
        logger.error("episode_data error: %s", exc)
        raise HTTPException(status_code=500, detail="Внутренняя ошибка")


@router.get("/episode/{episode_id}/progress")
async def episode_progress(
    episode_id: int,
    session: AsyncSession = Depends(get_session),
    user: Optional[User] = Depends(get_current_user),
):
    if not user:
        return {"timestamp": 0, "duration": 0}
    try:
        result = await session.execute(
            select(WatchProgress).where(
                WatchProgress.user_id == user.id,
                WatchProgress.episode_id == episode_id,
            )
        )
        wp = result.scalar_one_or_none()
        if not wp:
            return {"timestamp": 0, "duration": 0}
        return {"timestamp": wp.timestamp or 0, "duration": wp.duration or 0}
    except Exception:
        return {"timestamp": 0, "duration": 0}


@router.get("/anime/{anime_id}/progress")
async def anime_progress_summary(
    anime_id: int,
    session: AsyncSession = Depends(get_session),
    user: Optional[User] = Depends(get_current_user),
):
    if not user:
        return {"items": []}
    try:
        rows = await session.execute(
            select(WatchProgress, Episode)
            .join(Episode, Episode.id == WatchProgress.episode_id)
            .where(
                WatchProgress.user_id == user.id,
                Episode.anime_id == anime_id,
            )
        )
        items = []
        for wp, ep in rows.all():
            if not wp.duration:
                continue
            percent = int(min(100, max(0, (wp.timestamp / wp.duration) * 100)))
            items.append({
                "episode_id": ep.id,
                "episode_number": ep.episode_number,
                "percent": percent,
                "watched": percent >= 90,
            })
        return {"items": items}
    except Exception:
        return {"items": []}


_analytics_logger = logging.getLogger("animeflow.analytics")

_ALLOWED_EVENTS = {
    "play", "pause", "ended", "exit",
    "seek", "quality_change", "share", "fullscreen",
    "error", "double_tap_back", "double_tap_fwd",
}


class AnalyticsEvent(BaseModel):
    event: str
    anime_id: Optional[int] = None
    episode_id: Optional[int] = None
    episode_number: Optional[int] = None
    timestamp: Optional[float] = None
    duration: Optional[float] = None
    detail: Optional[str] = None


@router.post("/analytics/event")
async def analytics_event(
    payload: AnalyticsEvent,
    request: Request,
    user: Optional[User] = Depends(get_current_user),
):
    if payload.event not in _ALLOWED_EVENTS:
        raise HTTPException(status_code=400, detail="unknown event")
    try:
        user_part = "u={}".format(user.id) if user else "anon"
        pos = ""
        if payload.timestamp is not None and payload.duration:
            pct = int(min(100, max(0, (payload.timestamp / payload.duration) * 100)))
            pos = " pct={} t={:.1f}/{:.1f}".format(pct, payload.timestamp, payload.duration)
        detail = " detail={!r}".format(payload.detail) if payload.detail else ""
        _analytics_logger.info(
            "ev=%s anime=%s ep=%s %s%s%s",
            payload.event, payload.anime_id, payload.episode_id,
            user_part, pos, detail,
        )
    except Exception:
        pass
    return {"ok": True}
