...
async def login_submit(
    ...
    # Update login streak
    try:
        from app.services.achievements import update_login_streak
        await update_login_streak(user.id, session)
    except Exception:
        pass
    return RedirectResponse("/profile", status_code=303)
...
async def register_submit(
    ...
    request.session["user_id"] = user.id
    # Grant welcome achievement immediately
    try:
        from app.services.achievements import check_and_unlock
        from database import SessionLocal
        async with SessionLocal() as ach_session:
            await check_and_unlock(user.id, ach_session)
    except Exception:
        pass
    return RedirectResponse("/profile", status_code=303)
